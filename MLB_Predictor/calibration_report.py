...
Read-only analysis of MLB_Predictor/predictions_log.jsonl.
Writes nothing, commits nothing -- safe to run any time.
...
CALIBRATED_KEYS = ["pred_prob_cal", "hr_prob", "hr_over_05", "calibrated_prob", "prob", "predicted"]
RAW_KEYS = ["pred_prob", "hr_raw", "hr_raw_pct", "raw_prob"]
...
def report_for(label, pairs):
    ...
    if abs(gap) < 0.01:
        ...
        print(f"  Overall bias:   OVERPREDICTS by {gap:.1%}")
    else:
        print(f"  Overall bias:   UNDERPREDICTS by {-gap:.1%}")
    ...
    if skill > 0.001:
        print(f"  Skill:          +{skill:.1%} better than baseline.")
    elif skill < -0.001:
        print(f"  Skill:          {skill:.1%} WORSE than baseline.")
        print("                  Per-player differentiation is currently costing accuracy.")
    else:
        print("  Skill:          indistinguishable from baseline.")
...
def main():
    ...
    print(f"\nKeys present: {sorted(rows[0].keys()) if rows else 'none'}")
    ...
    if not cal_key and not raw_key:
        print("\n[ERROR] Couldn't find a probability field.")
        print("Keys in your log:", sorted(rows[0].keys()) if rows else "none")
        sys.exit(1)
    ...
    if cal_key and raw_key:
        cb = brier(pairs_for(cal_key))
        rb = brier(pairs_for(raw_key))
    ...
    print(f"\n{'=' * 72}")
    print("Note: gaps under ~3 points are within noise at this sample size.")


if __name__ == "__main__":
    main()"""calibration_report.py
...
CALIBRATED_KEYS = ["pred_prob_cal", "hr_prob", "hr_over_05", "hr_over_0.5", "HR Over 0.5%", "hr_p", "calibrated_prob", "prob", "predicted"]
                   "hr_p", "calibrated_prob", "prob", "predicted"]
RAW_KEYS = ["pred_prob", "hr_raw", "hr_raw_pct", "HR Raw%", "hr_p_raw", "raw_prob"]
